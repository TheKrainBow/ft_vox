...
void Chunk::loadBlocks()
{
    ...
    for (auto& f : futures) {
        ...
        // grab (and temporarily detach) any existing lazy subchunk for this idx
        SubChunk* existing = nullptr;
        {
            ...
            if (it == _subChunks.end()) {
                ...
                // swap in the generated one so readers see terrain immediately
                existing = it->second;
                it->second = generated;
            }
        }

        // If we had a pre-existing (lazy/edited) subchunk, merge its non-AIR voxels
        if (existing) {
            for (int y = 0; y < CHUNK_SIZE; y += _resolution) {
                for (int z = 0; z < CHUNK_SIZE; z += _resolution) {
                    for (int x = 0; x < CHUNK_SIZE; x += _resolution) {
                        ...
                        if (b != AIR) {
                            // write into the new generated subchunk
...
TopBlock Chunk::getTopBlock(int localX, int localZ)
{
    ...
    for (int subY = maxIdx; subY >= 0; --subY) {
        auto it = _subChunks.find(subY);           // << use find, not operator[]
...
TopBlock Chunk::getTopBlockUnderPlayer(int localX, int localY, int localZ)
{
    ...
    for (int subY = maxIdx; subY >= 0; --subY) {
        auto it = _subChunks.find(subY);           // << same fix
        if (it == _subChunks.end() || !it->second) continue;
...
void Chunk::getNeighbors()
{
    ...
    // link back to me (use shared_from_this)
...
SubChunk* Chunk::getOrCreateSubChunk(int subY, bool generate)
{
    ...
    if (generate) {
        // full terrain generation (may call setBlock → OK, we’re not holding _subChunksMutex)
        sc->loadHeight(_resolution);
        sc->loadBiome (_resolution);
    } else {
        // lazy: created to accept edits (tree logs/leaves).
        // mark as loaded so meshing includes it.
        sc->markLoaded(true);
    }
...
void Chunk::sendFacesToDisplay()
{
    ...
    // Take a snapshot of subchunks under _subChunksMutex
    ...
    // Now safe to build while holding only _sendFacesMutex
...
{
    // REMOVE: this makes height/tree maps change with LOD
    // _world.updatePerlinMapResolution(_perlinMap, newResolution);

    _resolution = newResolution;

    // snapshot subchunks and rebuild outside the lock (as you already do)
    std::vector<SubChunk*> subs;
    {
        std::lock_guard<std::mutex> lk(_subChunksMutex);
        subs.reserve(_subChunks.size());
    ...
    for (auto* sc : subs)
        sc->updateResolution(newResolution, _perlinMap);
